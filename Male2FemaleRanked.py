...
input_original_males = open('g6pd.merge.combined_positions.positive_strand.haps.males.nozeroes', 'r')
xsum = 0
ysum = 0
counter = {}
genotype_updated = ""
        ...
        for possible_matches in matches_score_dict.keys():
            ...
            if double_wammy == {}:
                ...
                if len(answer_one) > 1:
                    ...
                    answer_one = str(answer_one)
                    answer_one = answer_one[2:-2]
                    male_1_list = list(answer_one)
                    top_pick_mate = [int(c) - int(d) for c, d in zip(key_list, answer_one)]
                    top_pick_mate_string = ''.join(str(i) for i in top_pick_mate)
                    male_1_string = ''.join(str(i) for i in male_1_list)
                    results_tup_o = (male_1_string, top_pick_mate_string)
                    return results_tup_o
            else:
                x = double_wammy.keys()
...
for line in output:
    ...
    if data[1] == '[\'Error 1\']' or data[1] == '[\'Error 2\']':
        continue
    else:
...
for line in output_three:
    ...
    if final_data[1] == '[\'Error 1\']' or final_data[1] == '[\'Error 2\']':
        continue
    else:
        existing_value_1 = []
        existing_value_2 = []
